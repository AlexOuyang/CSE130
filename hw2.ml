...
let h' = f b in
if h' = b then (h', false)
else (h', true) (* if true, will recurse back to wwhile*)
in h
,b)
...
| ThreeSine(a, b, c) -> "sin(pi*"^exprToString a^"*"^"sin(pi*"^exprToString b^"*"^"sin(pi*"^exprToString c^")"
| Subtract(a, b)     -> "("^exprToString a^"-"^exprToString b^")"
...
let buildThreeSine(e1,e2,e3)       = ThreeSine(e1,e2,e3)
...
| Thresh(a,b,a',b')  -> if eval(a',x,y) < eval(b',x,y)
then eval(a,x,y) else eval(b,x,y)
| ThreeSine(a, b, c) -> sin(pi *. eval(a,x,y)) *. sin(pi *. eval(b,x,y)) *. sin(pi *. eval(c,x,y))
...
let sampleExpr = buildCosine(buildSine(buildTimes(buildCosine(buildAverage(buildCosine(buildX()),buildTimes(buildCosine (buildCosine (buildAverage(buildTimes (buildY(),buildY()),buildCosine (buildX())))),buildCosine (buildTimes (buildSine (buildCosine(buildY())),buildAverage (buildSine (buildX()), buildTimes(buildX(),buildX()))))))),buildY())))
...
if depth > 0 then (* only proceed if depth is not 0 *)
let r = rand(0, 11) in
match r with 
...
| 6  -> buildAverage(build(rand, depth-1), buildX())
| 7  -> buildAverage(build(rand, depth-1), buildX())
| 8  -> buildTimes(build(rand, depth-1), buildY())
| 9  -> buildTimes(build(rand, depth-1), buildY())
| 10 -> buildThresh(build(rand, depth-1), build(rand, depth-1), buildX(), buildY())
| 11 -> buildThresh(build(rand, depth-1), build(rand, depth-1), buildX(), buildY())

| _  -> buildSubtract(build(rand, depth-1), build(rand, depth-1))
else 
let r' = rand(0, 1) in
match r' with
| 0 -> VarX
