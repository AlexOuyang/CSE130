...
let times = abs (l1_length - l2_length) in
let x = clone 0 times in
if l1_length < l2_length then (x @ l1, l2)
else (l1, x @ l2)
;;
...
(c, r::res) in
let base = (0, []) in
let r1 = List.rev (0::l1) in
let r2 = List.rev (0::l2) in
let args = List.combine r1 r2 in
...
let (pad, res) = a in
let (num, digit) = x in
let p = num @ (clone 0 pad) in
let product = mulByDigit digit p in
let sum = bigAdd res product in
(pad + 1, sum) in
let base = (0, []) in
let len = List.length l2 in
let rl = List.rev l2 in
let c = clone l1 len in
let args = List.combine c rl in 
let (_, res) = List.fold_left f base args in
res
;;
